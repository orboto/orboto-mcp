/**
 * ORB-244 Phase A reference tool — `orbit_list_projects`.
 *
 * Maps to `GET /projects` in the Orbit API, which already filters by
...
export const listProjectsToolConfig = {
  title: 'List projects',
  description:
    'Return every Orbit project the authenticated user can see. Useful as the first step of any workflow ("which project should I look at?").',
  // Empty input schema — the tool has no required arguments; the
  // Orbit API already scopes results by the calling user's PBAC
  // cascade.
