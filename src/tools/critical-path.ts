...
export function makeCriticalPathHandler(client: OrbotoClient) {
  return async (input: { projectKey: string; milestone?: string }): Promise<CallToolResult> => {
    const project = await resolveProjectByKey(client, input.projectKey);
    let url = `/projects/${project.id}/critical-path`;
    if (input.milestone) {
      ...
      url += `?milestoneId=${encodeURIComponent(m.id)}`;
    }
