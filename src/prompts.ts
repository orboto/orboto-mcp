      ...
      messages: [{
        ...
        content: {
          ...
          text: [
            ...
            '',
            'Steps:',
            `1. Use \`orbit_get_project\` on "${projectKey}" to see milestones, members, and overall status.`,
            `2. Use \`orbit_list_tickets\` with statusCategory="todo" to fetch the open backlog.`,
            `3. Use \`orbit_list_milestones\` and pick the next not-yet-completed milestone as the sprint target.`,
      ...
      messages: [{
        ...
        content: {
          ...
          text: [
            ...
            '',
            'Steps:',
            '1. Use `orbit_my_tickets` (no statusCategory filter — defaults to open work) to fetch what I\'m assigned to.',
            '2. For each ticket, fetch full context with `orbit_get_ticket` to see description, latest comment, blocking sub-tickets, and linked git activity.',
            '3. Rank by:',
      ...
      messages: [{
        ...
        content: {
          ...
          text: [
            ...
            '',
            'Steps:',
            `1. Use \`orbit_get_project\` on "${projectKey}" to see metadata, milestones, members.`,
            `2. Use \`orbit_list_tickets\` with statusCategory="in_progress" to see what\'s actively being worked.`,
            `3. Use \`orbit_list_milestones\` to see what\'s nearest the deadline.`,
            '',
      ...
      messages: [{
        ...
        content: {
          ...
          text: [
            ...
            '',
            'Steps:',
            `1. Use \`orbit_get_ticket\` on "${ticketKey}" to read the description, type, priority, and any sub-tickets.`,
            `2. Use \`orbit_search\` with the ticket\'s title and description to find 3-5 similar past tickets, ideally already closed.`,
            '3. For each similar ticket found, use `orbit_get_ticket` to look up its loggedMinutes and check if the work seems comparable.',
      ...
      messages: [{
        ...
        content: {
          ...
          text: [
            ...
            '',
            'Steps:',
            `1. Use \`orbit_get_ticket\` on "${ticketKey}" to read its title and description.`,
            '2. Pull 3-5 keyword phrases from the description.',
            `3. Use \`orbit_search\` with each phrase to find candidate matches (limit each search to 5).`,
            '4. Use `orbit_get_ticket` on the top 3 candidates to verify the actual overlap.',
            '5. Output:',
